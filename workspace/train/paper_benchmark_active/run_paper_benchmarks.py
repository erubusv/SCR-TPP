...
RESULTS_PATH = PAPER_SUITE / "results" / "paper_benchmark_results_hetero_source_2000_adjusted.json"
...
class _SieveSupportEvaluator:
    ...
    def _infer_pricing_atom_cache_budget(self) -> int:
        ...
        if self.device.type == "cuda":
            try:
                ...
                if int(free_bytes) <= 2 * gib:
                    return 0
                return int(min(12 * gib, 0.45 * int(free_bytes)))
            except Exception:
                return int(4 * gib)
        return int(4 * gib)
    ...
    def _precompute_basis_responses(self) -> None:
        ...
        for src in self.source_ids:
            ...
            z_all = rd.witness_response_torch_batched(combined, eye)
            n_ev, n_gr = sizes
            event_basis = z_all[:, : int(n_ev)].contiguous()
            grid_basis = z_all[:, int(n_ev): int(n_ev) + int(n_gr)].contiguous()
        ...
        with torch.no_grad():
            for idx, sign_rules in sorted(rules_by_idx.items(), key=lambda item: int(item[0])):
                if not tuple(int(src) for src in self.subsets[int(idx)]):
                    continue
                event_sums_all, grid_atoms_all, grid_scope = self._pricing_atom_products(int(idx))
                if int(grid_scope.sum().item()) == 0:
                    continue
                scoped_weight = fisher_weight[grid_scope]
                upper_gain_by_sign = {"exc": 0.0, "inh": 0.0}
                grid_sum = torch.sum(grid_atoms_all * scoped_weight.reshape(1, -1), dim=1)
                signed_score = event_sums_all - grid_sum
                info = torch.sum(grid_atoms_all * grid_atoms_all * scoped_weight.reshape(1, -1), dim=1)
