"""Research sweeps for stagewise residual selectors using continuous-time cell rates.

This variant keeps the current best stagewise/partition-search structure, but replaces
event-time class logits with a more principled point-process quantity:
for each component cell, eta(bits) = log N_t(bits) - log T(bits), where N_t(bits) is
the number of target events whose component state equals bits and T(bits) is the
continuous-time occupancy of that state measured on the integration grid.
"""
...
from workspace.train.research_hier_cell_selectors import build_rows
...
def component_rate_stats(Xev: np.ndarray, y_event: np.ndarray, Xgr: np.ndarray, grid_w: np.ndarray):
    d = Xev.shape[1]
...
def main():
    ...
    ap.add_argument("--thr_values", default="0.12,0.15,0.2,0.25")
    ap.add_argument("--support_pow_values", default="0.5,1.0,1.25,1.5")
    ap.add_argument("--sup_pen_values", default="0.25,0.5,1.0,2.0")
    ...
    for thr, support_pow, sup_pen, components in itertools.product(
        ...
        Xev_tr = (tr_cache["event_q"] > float(thr)).astype(np.int8)
        Xgr_tr = (tr_cache["grid_q"] > float(thr)).astype(np.int8)
        Xev_va = (va_cache["event_q"] > float(thr)).astype(np.int8)
        Xgr_va = (va_cache["grid_q"] > float(thr)).astype(np.int8)
        base_tables = {}
        for comp_id, comp in enumerate(components):
            idxs = [local_by_source[s] for s in comp]
            eta_tr, support_tr = component_rate_stats(
                Xev_tr[:, idxs],
                tr_cache["event_y"],
                Xgr_tr[:, idxs],
                tr_cache["grid_w"],
            )
            eta_va, support_va = component_rate_stats(
                Xev_va[:, idxs],
                va_cache["event_y"],
                Xgr_va[:, idxs],
                va_cache["grid_w"],
            )
            # Conservative combination: only keep the shared part of the rate contrast.
            base_tables[comp_id] = np.sign(eta_tr) * np.minimum(np.abs(eta_tr), np.abs(eta_va))
            base_tables[f"support_{comp_id}"] = np.minimum(support_tr, support_va)
